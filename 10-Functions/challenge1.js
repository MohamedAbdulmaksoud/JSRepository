...
const poll = {
  ...
  registerNewAnswer() {
    ...
    optionsString += "(Write option number)";
  ...
  displayResults(type = "array") {
    if (typeof type === "string") {
      ...
      for (let i = 0; i < this.answers.length - 1; i++) {
        answersString += this.answers[i] + ",";
      }
      answersString += this.answers[this.answers.length - 1];
      console.log(`The poll results are ${answersString}`);
